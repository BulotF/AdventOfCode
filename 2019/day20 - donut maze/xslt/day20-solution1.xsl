    ...
    <xsl:template match="/">
        <xsl:call-template name="calculate-distances">
            ...
            <xsl:with-param name="intersections">
                <intersections>
                    <intersection line="{//distances/cell[@text='AA']/@line}" column="{//distances/cell[@text='AA']/@column}" distance="0"/>
                </intersections>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
                    ...
                    <intersections>
                        <xsl:for-each select="$intersections//intersection[@distance=$current-distance]">
                            ...
                            <xsl:for-each select="$root//distances/cell[@line=$current-line and @column=$current-column]/cell">
                                <intersection line="{@line}" column="{@column}" distance="{$current-distance + number(@distance)}"/>
                            </xsl:for-each>
                        </xsl:for-each>
                    </intersections>
                </xsl:variable>
                <xsl:variable name="total-intersections">
                    <intersections>
                        <xsl:for-each select="$intersections//intersection">
                            <xsl:if test="not($new-intersections//intersection[@line=current()/@line and @column=current()/@column and @distance &lt; current()/@distance])">
                                <xsl:copy-of select="."/>
                            </xsl:if>
                        </xsl:for-each>
                        <xsl:for-each select="$new-intersections//intersection">
                            <xsl:if test="not($intersections//intersection[@line=current()/@line and @column=current()/@column and @distance &lt;= current()/@distance])
                                      and not($new-intersections//intersection[@line=current()/@line and @column=current()/@column and @distance &lt; current()/@distance])">
                                <xsl:copy-of select="."/>
                            </xsl:if>
                        </xsl:for-each>
                    </intersections>

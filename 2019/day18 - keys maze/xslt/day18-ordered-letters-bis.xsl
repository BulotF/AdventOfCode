...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:call-template name="add-letter">
            <xsl:with-param name="route" as="node()">
                <maze>
                    <journey last="-@_€" distance="0" path=""/>
                </maze>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template name="add-letter">
        ...
        <xsl:variable name="route-with-doubles">
            <maze>
                <xsl:for-each select="$route//journey">
                    <xsl:variable name="journey" select="." as="node()"/>
                    <xsl:for-each select="1 to string-length($journey//@last)">
                        <xsl:variable name="current-letter" select="substring($journey//@last,position(),1)"/>
                        <xsl:apply-templates select="$root//distances/cell[@text=$current-letter]">
                            <xsl:with-param name="journey" select="$journey" as="node()"/>
                        </xsl:apply-templates>                        
                    </xsl:for-each>
                </xsl:for-each>
            </maze>
        ...
        <xsl:choose>
            <xsl:when test="$route-without-doubles//journey[1]/string-length(@path) = 26">
                ...
                <xsl:copy-of select="$route-without-doubles"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="add-letter">
    ...
    <xsl:template match="distances/cell">
        ...
        <xsl:for-each select="descendant::cell[upper-case(@text) != @text
            ...
            <journey last="{replace($journey//@last,$master-letter,$new-letter)}">
